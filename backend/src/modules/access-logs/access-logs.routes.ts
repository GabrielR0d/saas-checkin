...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    const [data, total] = await Promise.all([
      prisma.accessLog.findMany({ where, skip: (page - 1) * limit, take: limit, include: { client: { select: { id: true, name: true, phone: true } }, device: { select: { id: true, name: true, location: true } } }, orderBy: { occurredAt: 'desc' } }),

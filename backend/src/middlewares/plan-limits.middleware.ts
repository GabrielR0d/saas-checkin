...
const LIMITS: Record<string, Record<Resource, number>> = {
...
export function planLimits(resource: Resource) {
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    try {
      ...
      const limit = LIMITS[tenant.plan]?.[resource] ?? 50
      let current = 0
      ...
      if (current >= limit) { res.status(403).json({ error: 'PLAN_LIMIT', currentPlan: tenant.plan, limit, current }); return }
      next()
    } catch (err) { console.error('[PlanLimits]', err); next() }
  }
}

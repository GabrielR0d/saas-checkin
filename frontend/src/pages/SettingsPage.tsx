...
export function SettingsPage() {
  ...
  useEffect(() => {
    ...
    fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(debouncedAddress)}&format=json&limit=1&countrycodes=br`, {
      headers: { 'Accept-Language': 'pt-BR', 'User-Agent': 'saas-checkin/1.0' },
    })
      .then((r) => r.json())
      .then((results) => {
        if (results.length > 0) {
